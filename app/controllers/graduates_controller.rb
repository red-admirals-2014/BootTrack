class GraduatesController < ApplicationController
  ...
  def get_graduates(campus, year)
    ...
    graduates = Cohort.joins(:graduates).order('start_date DESC').where('extract(year  from start_date) = ?', year).limit(30) if campus=='default' && year!='default'
    graduates = Cohort.joins(:graduates).order('start_date DESC').where(location: campus).where('extract(year  from start_date) = ?', year).limit(30) if campus!='default' && year!='default'
    return graduates
  end
